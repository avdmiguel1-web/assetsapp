...
import { formatDateRangeValue, formatTimeRangeValue, getRentalRangeKind, isRentalLocationName } from "../lib/locationUtils";
...
function TransferModal({ open, onClose }) {
  ...
  const selectedDest = locations.find((location) => location.id === toLocationId) || null;
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter((asset) =>
      (asset.assetId && asset.assetId.toLowerCase().includes(q))
  ...
  const handleTransfer = async () => {
    ...
    if (!selectedAsset) {
      setError(t.transfers.errorAsset);
      return;
    }
        ...
        <div className="modal-body" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            {assets.length === 0 ? (
              ...
              <div style={{ position: "relative" }}>
                ...
                >
                  ...
                  <input
                    placeholder={t.transfers.assetSearchPlaceholder || "Buscar por ID activo, marca o modelo..."}
                    value={assetSearch}
                  ...
                  {(assetSearch || selectedAsset) && (
                    <button className="btn btn-ghost btn-icon btn-sm" onClick={handleClearAsset} title={t.transfers.clearSelection || "Limpiar selección"}>
                      <X size={12} />
                    </button>
                  )}
                </div>
                  ...
                  >
                    {filteredAssets.length === 0 ? (
                      ...
                      filteredAssets.map((asset) => (
                        ...
                        >
                          {asset.profilePhoto
                            ? <ResolvedImage src={asset.profilePhoto} alternateSrc={asset.profilePhotoSource} alt="" style={{ width: 32, height: 32, borderRadius: "var(--radius-sm)", objectFit: "cover", flexShrink: 0 }} />
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 11, color: "var(--text-muted)", display: "flex", gap: 6, marginTop: 1, flexWrap: "wrap" }}>
                              {asset.assetId && <span style={{ fontFamily: "'IBM Plex Mono',monospace" }}>{asset.assetId}</span>}
                              {asset.plate && <span>· {asset.plate}</span>}
          ...
          <div className="form-group">
            ...
            {locations.length === 0 ? (
              ...
              <select className="form-select" value={toLocationId} onChange={(event) => { setToLocationId(event.target.value); setError(""); }} disabled={!selectedAsset}>
                <option value="">{t.transfers.selectDest}</option>
                {destOptions.map((location) => (
                  <option key={location.id} value={location.id}>{FLAG_MAP[location.country] || ""} {location.name} — {location.address || location.country}</option>
                ))}
              </select>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div className="form-grid">
                <div className="form-group">
                  ...
                  <input className="form-input" type="date" value={rentalStartDate} onChange={(event) => { setRentalStartDate(event.target.value); setError(""); }} />
                </div>
                <div className="form-group">
                  <label className="form-label">{t.transfers.rentalDateToLabel || "Fecha hasta"}</label>
                  <input className="form-input" type="date" value={rentalEndDate} onChange={(event) => { setRentalEndDate(event.target.value); setError(""); }} />
                </div>
              </div>
              <div className="form-grid">
                <div className="form-group">
                  <label className="form-label">{t.transfers.rentalTimeFromLabel || "Hora desde"}</label>
                  <input className="form-input" type="time" value={rentalStartTime} onChange={(event) => { setRentalStartTime(event.target.value); setError(""); }} />
                </div>
                <div className="form-group">
                  <label className="form-label">{t.transfers.rentalTimeToLabel || "Hora hasta"}</label>
                  <input className="form-input" type="time" value={rentalEndTime} onChange={(event) => { setRentalEndTime(event.target.value); setError(""); }} />
                </div>
              </div>
              ...
              <div>
                ...
                {isRentalDestination && (rentalStartDate || rentalEndDate) && (
                  <div style={{ fontSize: 11, color: "var(--accent-green)", marginTop: 4 }}>
                    {(t.transfers.rentalDateLabel || "Fecha alquiler")}: {formatDateRangeValue(rentalStartDate, rentalEndDate, locale) || "—"}
                  </div>
                )}
                {isRentalDestination && (rentalStartTime || rentalEndTime) && (
                  <div style={{ fontSize: 11, color: "var(--accent-green)", marginTop: 4 }}>
                    {(t.transfers.rentalTimeLabel || "Hora alquiler")}: {formatTimeRangeValue(rentalStartTime, rentalEndTime, locale) || "—"}
                  </div>
                )}
              </div>
            </div>
...
export default function TransfersPageFixed() {
  const t = useT();
  ...
  const filtered = useMemo(() => {
    ...
    return transfers.filter((transfer) => {
      ...
      if (!q) return true;
      return `${transfer.fromLocation} ${transfer.toLocation} ${transfer.fromCountry} ${transfer.toCountry} ${transfer.fromAddress || fromLocation?.address || ""} ${transfer.toAddress || toLocation?.address || ""} ${asset?.assetId || transfer.assetId} ${asset?.brand || ""} ${asset?.model || ""}`
        .toLowerCase()
        .includes(q);
    });
  ...
  const enriched = filtered.map((transfer) => {
    ...
    return {
      ...transfer,
      asset: assets.find((asset) => asset.id === transfer.assetId),
      ...
      {transfers.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div className="search-bar">
            ...
            <input
              placeholder={t.transfers.searchPlaceholder || "Buscar por ID activo, marca, modelo, origen, destino..."}
              value={search}
