...
export default function AssetDetailModalFixed({ open, onClose, asset, onEdit }) {
  ...
  const currentLocation = locations.find((location) => location.id === asset.locationId)
    || locations.find((location) => location.name === asset.location && location.country === asset.country);
  const rentalInfo = {
    rentalStartDate: asset.rentalStartDate || currentLocation?.rentalStartDate || "",
    rentalEndDate: asset.rentalEndDate || currentLocation?.rentalEndDate || "",
